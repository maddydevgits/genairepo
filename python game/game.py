...
class Game:
    ...
    def spawn_insects(self):
        ...
        if t > self.insects_spawn_timer:
            self.insects_spawn_timer = t + blueS_SPAWN_TIME

            # increase the probability that the insect will be a red over time
            ...
            if random.randint(0, 100) < nb:
                self.insects.append(red())
            else:
                self.insects.append(blue())

            # spawn a other blue after the half of the game
